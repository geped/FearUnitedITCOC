...
function showApp(user) {
    ...
    if (isAdmin) {
        ...
        document.querySelectorAll('.admin-only').forEach(el => {
            ...
            el.style.display = (tag === 'button' || tag === 'span') ? 'inline-block' : 'block';
        });
...
document.getElementById('generate-bonus').addEventListener('click', async () => {
    ...
    if (cwlLiveData && cwlLiveData.length) {
        ...
        if (error || !history?.length) {
            msg.textContent = '⚠ Nessun dato per questa stagione. Carica prima lo storico CWL o i dati live.';
            return;
        }
    ...
    bonusSelections = new Set(bonusCandidates.slice(0, count).map(c => c.player_name));
...
function renderBonusPlayerList() {
    ...
    if (!bonusCandidates.length) { div.innerHTML = ''; return; }

    const rows = bonusCandidates.map((c, i) => {
        ...
        const avgDestr = c.attacks_made > 0 ? (c.destruction / c.attacks_made).toFixed(1) + '%' : '—';
