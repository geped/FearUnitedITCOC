...
--
-- Ordine di applicazione: 1→base, 2→update, 3→cwl, 4→bonus, 5→multiclan, 6→retention, 7→league, 8→classic-wars, 9→security, 10→soft-delete, 11→cwl-wars
--
...
-- ═══ SEZIONE 11: CWL-WARS — Storico guerre CWL con dettaglio attacchi ═══
-- Fonte: schema-cwl-wars.sql
...
-- ═══ SEZIONE 12: CAPITAL-RAIDS — Storico weekend raid capitale ═══
-- Fonte: schema-capital-raids.sql
...
CREATE POLICY "capital_raids_read" ON public.capital_raids
    FOR SELECT USING (true);

CREATE POLICY "capital_raids_insert" ON public.capital_raids
    FOR INSERT WITH CHECK (true);

CREATE POLICY "capital_raids_update" ON public.capital_raids
    FOR UPDATE USING (true);
